...
export interface ICommentRepository {
  ...
  findByPostId(
    ...
    limit: number,
    cursor?: string,
  ): Promise<{ items: IComment[]; nextCursor: string | null }>;
  findReplies(
    parentId: string,
    limit: number,
    cursor?: string,
...
export interface IStoryRepository {
  create(
    story: Omit<
      IStory,
      'id' | 'stats' | 'isDeleted' | 'createdAt' | 'updatedAt'
    >,
  ): Promise<IStory>;
