...
export class CoursesService {
  ...
  private calculateStreak(
    ...
    for (
      ...
      const previous =
        new Date(
          `${uniqueDates[index - 1]}T00:00:00.000Z`,
        );

      const current =
        new Date(
          `${uniqueDates[index]}T00:00:00.000Z`,
        );
  ...
  private summariseLearningProgress(
    course: any,
    enrollment: any,
  ) {

    const totalClasses =
      this.getCourseTotalClasses(
        course,
      );
  ...
  private async getVdoCipherPlaybackData(
    videoId: string,
  ) {
...
async getClassView(
  ...
  if (!purchased) {
    ...
    throw new NotFoundException(
      'Purchase course to access class',
    );
  ...
  const vdoCipher =
    await this.getVdoCipherPlaybackData(
      cls.videoId,
    );
...
async updateClassProgress(
  ...
  enrollment.learningProgress =
    this.summariseLearningProgress(
      course,
      enrollment,
    );
